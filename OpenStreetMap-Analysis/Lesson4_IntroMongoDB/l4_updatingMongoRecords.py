...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        data = [x for x in data if x['binomialAuthority'] != 'NULL']
...
def update_db(data, db):
    for el in data:
        spider = db.arachnid.find_one({'label': el['label']})
        spider['classification']['binomialAuthority'] = el['binomialAuthority']
        db.arachnid.save(spider)

#### last step is putting binom auth in the classification field rather than at level 1
